...
public class BallManip extends Component
{
    ...
    public BallManip()
    {
        ...
        armMover.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0, 30);
    ...
    public void update()
    {
        ...
        //{
            ...
            //SmartDashboard.putNumber("Arm Encoder", armMover.getSelectedSensorPosition());
            ...
            if(Math.abs(RobotMap.manipController.getRawAxis(XboxMap.RIGHT_JOY_VERT)) > DEADZONE_2)
            {
                ...
                else if(SwitchMode.mode != 'H')
                {
                    armMover.set(-.7*RobotMap.manipController.getRawAxis(XboxMap.RIGHT_JOY_VERT));
                }
            }
            else if (RobotMap.manipController.getRawButton(XboxMap.RB))
            {
                armMover.set(.25);
            }
            else
    ...
    {        
        ...
        if(limitSwitchBack.get() && (RobotMap.manipController.getTriggerAxis(Hand.kLeft) > DEADZONE_2 && RobotMap.manipController.getTriggerAxis(Hand.kRight) < DEADZONE_2))
         {
            //Succ ball in
            leftSuck.set(.7*RobotMap.manipController.getTriggerAxis(Hand.kLeft) * MULTIPLIER);
            rightSuck.set(.7*RobotMap.manipController.getTriggerAxis(Hand.kLeft) * MULTIPLIER);
         }
