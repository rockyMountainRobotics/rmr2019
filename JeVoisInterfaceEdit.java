...
public class JeVoisInterfaceEdit {
    ...
    //character array to temp store buffer in (rmr662 edit)
    char[] ri_char;
     ...
     */
    public JeVoisInterfaceEdit(boolean useUSBStream) {
        int retry_counter = 0;
    ...
    private String blockAndGetPacket(double timeout_s){
                ...
                if (visionPort.getBytesReceived() > 0) {
                    ...
                    if(packetBuffer.indexOf(PACKET_START_CHAR) == -1){
                        ...
                        if(endIdx == -1){
                            ...
                            //allows the program to break out of loop because it will never reach above else
                            break; 
                        } else {
        ...
        //character array to temp store buffer in (rmr662 edit)
        ri_char = new char[packetBuffer.length()];

        System.out.println("Flag 1");

        if(retval == null)
        {
            packetBuffer.getChars(0,packetBuffer.length(), ri_char, 0);
            for(int i=0; i<packetBuffer.length(); i++)
            {
                System.out.print(ri_char[i]);
            }
        }
        return retval;
    }
